# ** NOTE ON DIFFRENCE BETWEEN 'en_core_web_sm' AND 'en_core_web_md'
# 'en_core_web_sm' is a smaller model which uses no word vectors loaded so the result odf the Doc.similarity method will be based on the tagger, parser, and NER, which may not give useful similarity judgements as opposed to 'en_core_web_md' which has these items already included which will result in more meaningful judgements.
